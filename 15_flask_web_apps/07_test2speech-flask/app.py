...
openai = OpenAI(api_key="sk-proj-ezHgiooTJpbgkCiW3JO9T3BlbkFJ55Iw60OTMZaZlzHA0RUW")
...
def convert():
    ...
    if format != "mp3":
        with tempfile.NamedTemporaryFile(delete=False, suffix=f".{format}") as tmpfile:
            convert_audio_format(mp3_speech_path, tempfile.name, format)
...
# add the name option to save the file
